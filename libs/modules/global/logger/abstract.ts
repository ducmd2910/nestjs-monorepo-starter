...
export abstract class AbstractLoggerService<T extends HttpLogger = HttpLogger> {
  abstract pino: T;
  abstract connect<TLevel = LevelWithSilent>(logLevel?: TLevel): void;
  abstract setApplication(app: string): void;
  /**
   * @deprecated The method should be use only in main.ts, this log won't be saved in elastic, only stdout
   */
